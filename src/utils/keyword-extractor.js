...
exports.extractJiraKeywords = extractJiraKeywords;
exports.loadPageIds = loadPageIds;
exports.extractKeywordsWithAdaptiveCount = extractKeywordsWithAdaptiveCount;
exports.runKeywordExtraction = runKeywordExtraction;
const fs = __importStar(require("fs-extra"));
const path = __importStar(require("path"));
const environment_1 = require("../environment");
// Cache for JIRA content to avoid multiple file reads
let jiraContentCache = {};
/**
 ...
 */
async function extractJiraKeywords(jiraFilePath, keywordCount = 5) {
    try {
 ...
 */
async function savePageIds(filteredPages) {
    try {
        const pageIds = filteredPages.map(page => page.id);
        const pagesFilePath = path.join(process.cwd(), environment_1.ENV_VARS.TMP_DIR_PATH, 'pages.txt');
 ...
 */
async function extractKeywordsWithAdaptiveCount(pages, jiraFilePath) {
 ...
 */
function filterPagesByKeywords(pages, keywords) {
    return pages.filter(page => {
        const title = (page.title || '').toLowerCase();
        const content = (page.body?.storage?.value || '').toLowerCase();
        const searchableContent = `${title} ${content}`;
        return keywords.some(keyword => searchableContent.includes(keyword.toLowerCase()));
    });
}
/**
